...
void main()
{
    ...
    float spotlight_opening_angle = cos(radians(30.0));
    vec4 l = normalize(spotlight_position - p);

    // Vetor que define o sentido da reflexão especular ideal.
    vec4 r = -l + (2.0*n) * (dot(n, l)); // [TALVEZ] PREENCHA AQUI o vetor de reflexão especular ideal
    ...
    bool is_unreachable = dot(normalize(p - spotlight_position), normalize(spotlight_direction)) < spotlight_opening_angle;
    if (is_unreachable) {
        Kd = vec3(0.0, 0.0, 0.0);
        Ks = vec3(0.0, 0.0, 0.0);
    }
