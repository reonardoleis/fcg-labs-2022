...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
            ...
            PushMatrix(model); // Guardamos matriz model atual na pilha
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    ...
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        ...
                        // MAO DIREITA
                        model = model * Matrix_Translate(0.0f, -1.10f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com a translação do antebraço direito
                        PushMatrix(model);
        ...
        // Braço ESQUERDO
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(+0.55f, 0.0f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com uma translação para o braço esq
            PushMatrix(model); // Guardamos matriz model atual na pilha
                model = model // Atualizamos matriz model (multiplicação à direita) com a rotação do braço esq
                      * Matrix_Rotate_Z(-g_AngleZ)  // TERCEIRO rotação Z de Euler
                      * Matrix_Rotate_Y(g_AngleY)  // SEGUNDO rotação Y de Euler
                      * Matrix_Rotate_X(g_AngleX); // PRIMEIRO rotação X de Euler
                PushMatrix(model); // Guardamos matriz model atual na pilha
                ...
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Translate(0.0f, -0.65f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com a translação do antebraço esq
                    model = model // Atualizamos matriz model (multiplicação à direita) com a rotação do antebraço esq
                          * Matrix_Rotate_Z(-g_ForearmAngleZ)  // SEGUNDO rotação Z de Euler
                          * Matrix_Rotate_X(g_ForearmAngleX); // PRIMEIRO rotação X de Euler
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Scale(0.2f, 0.6f, 0.2f); // Atualizamos matriz model (multiplicação à direita) com um escalamento do antebraço seq
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                        DrawCube(render_as_black_uniform); // #### ANTEBRAÇO ESQ // Desenhamos o antebraço esq

                        // MAO ESQ
                        model = model * Matrix_Translate(0.0f, -1.10f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com a translação do antebraço esq
                        PushMatrix(model);
                            model = model * Matrix_Scale(1.0f, 0.15f, 1.0f); // Atualizamos matriz model (multiplicação à direita) com um escalamento do antebraço esq
                            glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                            DrawCube(render_as_black_uniform); // #### MAO ESQ // Desenhamos a mao esq
                        PopMatrix(model);
                        //FIM  MAO ESQ
        ...
        PushMatrix(model); // Guardamos matriz model atual na pilha
            model = model * Matrix_Translate(+0.20f, -1.05f, 0.0f);
            PushMatrix(model);
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Scale(0.3f, 0.65f, 0.35f);
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                    DrawCube(render_as_black_uniform); // #### perna ESQUERDa
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Translate(0.0f, -0.70f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com a translação da perna esq
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Scale(0.25f, 0.6f, 0.30f); // Atualizamos matriz model (multiplicação à direita) com um escalamento da perna esq
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                        DrawCube(render_as_black_uniform); // #### panturrilha ESQ // Desenhamos a panturrilha esq

                        // pé esq
                        model = model * Matrix_Translate(0.0f, -1.10f, 0.5f); // Atualizamos matriz model (multiplicação à direita) com a translação da  panturrilha esq
                        PushMatrix(model);
                            model = model * Matrix_Scale(0.9f, 0.15f, 2.0f); // Atualizamos matriz model (multiplicação à direita) com um escalamento da panturrilha esq
                            glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                            DrawCube(render_as_black_uniform); // #### pe ESQ // Desenhamos o pe esq
                        PopMatrix(model);
                        //FIM  pé ESQ
        ...
        PushMatrix(model); // Guardamos matriz model atual na pilha
            ...
            PushMatrix(model);
                ...
                PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    model = model * Matrix_Translate(0.0f, -0.70f, 0.0f); // Atualizamos matriz model (multiplicação à direita) com a translação da perna direita
                    PushMatrix(model); // Guardamos matriz model atual na pilha
                        model = model * Matrix_Scale(0.25f, 0.6f, 0.30f); // Atualizamos matriz model (multiplicação à direita) com um escalamento da perna direita
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
                        DrawCube(render_as_black_uniform); // #### panturrilha direita // Desenhamos a panturrilha direita

                        // pé esq
                        model = model * Matrix_Translate(0.0f, -1.10f, 0.5f); // Atualizamos matriz model (multiplicação à direita) com a translação da  panturrilha direita
                        PushMatrix(model);
                            model = model * Matrix_Scale(0.9f, 0.15f, 2.0f); // Atualizamos matriz model (multiplicação à direita) com um escalamento da panturrilha direia
                            glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model)); // Enviamos matriz model atual para a GPU
